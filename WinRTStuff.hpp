...
namespace WinRTStuff {
	...
	public:
		...
		RenderWindow(const wchar_t* WindowClassName, unsigned __int32 ViewPixelWidth, unsigned __int32 ViewPixelHeight,
			const wchar_t* WindowTitle = L"RenderWindow" ) : WindowHandle{ NULL } {
			unsigned __int32 TotalWidth{ ViewPixelWidth + 16u }, TotalHeight{ ViewPixelHeight + 39u };

			this->WindowHandle = CreateWindowEx(NULL, WindowClassName, WindowTitle,
				WS_VISIBLE | WS_SYSMENU | WS_MINIMIZEBOX, 0, 0, TotalWidth, TotalHeight, NULL, NULL, GetModuleHandle(NULL), NULL);
		...
		~RenderWindow() {
			if (IsWindowVisible(this->WindowHandle) == true) {
