...
namespace DirectXStuff {
	...
	public:
		...
		CommandQueue(ID3D12Device6* pDevice, D3D12_COMMAND_QUEUE_PRIORITY Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL,
			const wchar_t* ObjectName = L"CommandQueue", D3D12_COMMAND_LIST_TYPE QueueType = D3D12_COMMAND_LIST_TYPE_DIRECT,
	...
	public:
		...
		CommandAllocator(ID3D12Device6* pDevice, const wchar_t* ObjectName = L"CommandAllocator",
	...
	public:
		...
		Fence(ID3D12Device6* pDevice, const wchar_t* ObjectName = L"Fence") : FenceIncrement{ 1u }, FenceCurrentValue{},
			FenceEventHandle{} {
			HRESULT Result{ S_OK };
		...
		void FlushCommandQueue(ID3D12CommandQueue* pCommandQueue, DWORD WaitTimeInMilliseconds = INFINITE) {
			pCommandQueue->Signal(reinterpret_cast<ID3D12Fence1*>(this->pInterface), FenceCurrentValue + FenceIncrement);

			reinterpret_cast<ID3D12Fence1*>(this->pInterface)->SetEventOnCompletion(FenceCurrentValue + FenceIncrement,
				FenceEventHandle);

			WaitForSingleObject(FenceEventHandle, WaitTimeInMilliseconds);
	...
	public:
		...
		GraphicsCommandList(ID3D12Device6* pDevice, ID3D12CommandAllocator* pCommandAllocator,
			const wchar_t* ObjectName = L"GraphicsCommandList",
	...
	public:
		SwapChain(IDXGIFactory7* pFactory, ID3D12CommandQueue* pCommandQueue, HWND WindowHandle, unsigned __int32 PixelWidth,
			unsigned __int32 PixelHeight, unsigned __int32 BufferCount = 2u,
